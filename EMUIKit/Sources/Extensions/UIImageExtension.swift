...
public extension UIImage {
    ...
    func decodeImage() -> UIImage? {
        ...
        autoreleasepool { () -> Void in
            ...
            // calculated above.
            let data = UnsafeMutableRawPointer.allocate(byteCount: Int(self.size.width) * Int(self.size.height) * 4, alignment: 0)
    ...
    class func resizeImageFromFile(_ filePath: String, maxSize: CGFloat) -> UIImage? {
        ...
        if FileManager.default.fileExists(atPath: filePath), let imageSource = CGImageSourceCreateWithURL(imageURL as CFURL, nil) {
            ...
            if let imageRef = CGImageSourceCreateThumbnailAtIndex(imageSource, 0, options as CFDictionary?) {
    ...
    class func resizeImageFromFile(_ filePath: String, minSize: CGFloat) -> UIImage? {
        var result: UIImage?
        let imageURL = URL(fileURLWithPath: filePath)
        ...
        if FileManager.default.fileExists(atPath: filePath), let imageSource = CGImageSourceCreateWithURL(imageURL as CFURL, nil) {
            ...
            if let properties = CGImageSourceCopyPropertiesAtIndex(imageSource, 0, nil) as NSDictionary?,
                let widthValue = properties["PixelWidth"] as? NSNumber,
                let heightValue = properties["PixelHeight"] as? NSNumber {
                let width = CGFloat(widthValue.doubleValue)
                let height = CGFloat(heightValue.doubleValue)

                if width > 0 && height > 0 {
                    if width < height {
                        maxSize = minSize * height / width
                    } else {
                        maxSize = minSize * width / height
                    }
                }
            }
    ...
    func resizedImageToFitInSize(_ boundingSize: CGSize, scaleIfSmaller scale: Bool) -> UIImage? {
        ...
        switch orient {
        case UIImage.Orientation.left, UIImage.Orientation.right, UIImage.Orientation.leftMirrored, UIImage.Orientation.rightMirrored:
...
//        }
        ...
        let hasSaturationChange = abs(saturationDeltaFactor - 1.0) > Float.ulpOfOne
...
//            var effectCGImage = vImageCreateCGImageFromBuffer(&inputBuffer, &format, nil, nil, vImage_Flags(kvImageNoAllocate), nil).takeRetainedValue() as CGImage?
            ...
            if effectCGImage == nil {
                effectCGImage = vImageCreateCGImageFromBuffer(&inputBuffer, &format, { (_, bufferData) in
                    if bufferData != nil {
                        free(bufferData!)
                    }
