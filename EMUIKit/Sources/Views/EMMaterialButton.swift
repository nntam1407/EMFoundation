...
class EMMaterialButton: UIButton, CAAnimationDelegate {
    ...
    */
    ...
    override func setImage(_ image: UIImage?, for state: UIControl.State) {
        if image?.renderingMode != UIImage.RenderingMode.alwaysTemplate && state == UIControl.State() {
    ...
    private func setupCustomizeMaterialButton() {
        ...
        self.setImage(self.image(for: UIControl.State())?.withRenderingMode(UIImage.RenderingMode.alwaysTemplate), for: UIControl.State())
    }
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        EMDispatchUtils.dispatchAfterDelay(0.15, queue: DispatchQueue.main) { () -> Void in
