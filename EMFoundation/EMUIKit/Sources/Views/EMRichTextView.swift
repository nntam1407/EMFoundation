...
class EMRichTextView: UITextView {
    ...
    */
    ...
    override init(frame: CGRect, textContainer: NSTextContainer?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(EMRichTextView.notificationTextDidChanged(_:)), name: UITextView.textDidChangeNotification, object: nil)
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        if self.placeholder != nil && self.placeholder!.count > 0 {
            if self.placeholderLabel == nil {
                ...
                let placeholderRect = CGRect(
                    x: self.textContainerInset.left + linePadding,
                    y: self.textContainerInset.top,
                    width: rect.size.width - self.textContainerInset.left - self.textContainerInset.right - 2 * linePadding,
                    height: rect.size.height - self.textContainerInset.top - self.textContainerInset.bottom
                )
        ...
        if self.imageMapDictionary != nil && self.text.count > 0 {
            ...
            for key in self.imageMapDictionary!.allKeys as! [String] {
                ...
                while true {
                    ...
                    let one = searchAttr?.attributedSubstring(from: NSRange(location: 0, length: range.location))
                    let two = searchAttr?.attributedSubstring(from: NSRange(location: range.location + range.length, length: (searchAttr?.string.count)! - (range.location + range.length)))
