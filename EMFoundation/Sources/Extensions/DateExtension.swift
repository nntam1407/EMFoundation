...
public extension Date {
    var timeMillisecondsIntervalSince1970: TimeInterval {
...
public extension Date {
    ...
    func toElapsedTimeString() -> String {
        ...
        let timeComponents = Calendar.current.dateComponents(
            Set(arrayLiteral: Calendar.Component.year,
        ...
        if seconds! >= 1 {
            return NSLocalizedString("a moment later", comment: "")
        } else if seconds! < 1 {
    ...
    func toShortDateString(locale: Locale = .current) -> String {
        let dateFormatter = DateFormatter()
    ...
    static func dateFromShortDateString(_ dateString: String, locale: Locale = .current) -> Date? {
        let dateFormatter = DateFormatter()
        dateFormatter.locale = locale
        dateFormatter.dateStyle = .short
        dateFormatter.timeStyle = .none
    ...
    func toString(_ format: String, locale: Locale = .current) -> String {
        let dateFormatter = DateFormatter()
        dateFormatter.locale = locale
    ...
    func toString(dateStyle: DateFormatter.Style, timeStyle: DateFormatter.Style, locale: Locale = .current) -> String {
        let dateFormatter = DateFormatter()
        dateFormatter.locale = locale
